...
async def test_sign_up_is_refused_unless_it_is_turned_on(anonymous_client, registration) -> None:
...
async def test_the_mode_is_readable_without_signing_in(anonymous_client, registration) -> None:
...
async def test_approving_twice_is_harmless(anonymous_client, admin_client, registration) -> None:
    """A double-click must not move the approval date or fail."""
    registration("approval")
    user_id = (await anonymous_client.post("/api/auth/register", json=NEW_ACCOUNT)).json()["id"]
...
async def test_open_mode_lets_them_in_immediately(anonymous_client, registration) -> None:
    registration("open")
...
async def test_signing_up_cannot_make_you_an_admin(anonymous_client, registration) -> None:
...
async def test_only_an_admin_may_approve(anonymous_client, client, registration) -> None:
    registration("approval")
    user_id = (await anonymous_client.post("/api/auth/register", json=NEW_ACCOUNT)).json()["id"]
